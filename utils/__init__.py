...
def ols(X, Y):
    """Dead simple ordinary least squares regression function

    Parameters
    ----------
    X : array
        X (Independent variable) in regression; in fMRI studies, generally
        a description of the stimulus (time x conditions) or (time x features)
    Y : array
        Y (Dependent variable) in regression; in fMRI studies, usually BOLD
        activations, (time x voxels)

    Returns
    -------
    B : array
        regression weights (features x voxels) or (conditions x voxels)
    """
